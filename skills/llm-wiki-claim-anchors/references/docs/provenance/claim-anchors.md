...
![Diagram of an agent anchoring wiki claims to supporting sources with visible links.](../assets/preview/claim-anchors.webp)
...
```bash
node scripts/validate-claim-anchors.mjs
```
...
3. `node scripts/validate-claim-anchors.mjs` checks uniqueness and support lines.
