...
Treat routing metadata as a product surface, not as background documentation. A route should state:

```text
verb + object + intent boundary
```
...
Every route must have at least one case in [`../benchmarks/router-eval.json`](../benchmarks/router-eval.json). Each case is a user-like utterance plus `expected_skill`; `npm run validate:router` runs the deterministic harness and fails if routing drifts.
...
| [`llm-wiki-benchmark-suite`](../skills/llm-wiki-benchmark-suite/SKILL.md) | | | | no | | |
| [`llm-wiki-capture-pipeline`](../skills/llm-wiki-capture-pipeline/SKILL.md) | design capture pipeline architecture; design cross channel topology; define inbox raw flow; define capture metadata contract; define dedupe boundary; define triage handoff; set up telegram connector; set up email connector; set up voice note connector; write slack export runbook; configure specific channel connector; configure channel api | | low | no | design-proposal | wiki-triage, llm-wiki-ingestion-stack |
| [`llm-wiki-channel-capture`](../skills/llm-wiki-channel-capture/SKILL.md) | | | | no | | |
...
| [`llm-wiki-claim-anchors`](../skills/llm-wiki-claim-anchors/SKILL.md) | claim anchors; stable claim ids; source anchor ids; format support labels; validate claim anchors | missing sources; provenance gaps; unsupported claims; repair evidence links | medium | no | patch-after-evidence | llm-wiki-conflict-resolver |
...
| [`llm-wiki-eval`](../skills/llm-wiki-eval/SKILL.md) | define measurement; evaluate usefulness; quality metrics; retrieval hit rate; maintenance health; interpret eval results; pilot benchmark; run baseline pass; run with wiki pass; score pilot tasks; continue pause redesign decision | eval tools; ci eval gates; eval framework selection | low | no | report-only | llm-wiki-eval-tooling, llm-wiki-human-first-design, llm-wiki-critique-audit |
| [`llm-wiki-eval-tooling`](../skills/llm-wiki-eval-tooling/SKILL.md) | eval tools; ragas promptfoo deepeval; ci eval gates; choose scorecard; eval framework selection | pilot benchmark; run baseline pass; run with wiki pass; define measurement | none | yes | recommendation-with-citations | llm-wiki-eval |
...
| [`llm-wiki-faq`](../skills/llm-wiki-faq/SKILL.md) | why needed; benefits; evidence summary; proof question; obsidian required; living wiki; answer objection; stakeholder faq; concise rebuttal | risk report; failure mode register; residual risk scorecard; bad fit assessment | none | no | answer | llm-wiki-human-first-design, llm-wiki-critique-audit, llm-wiki-choose, llm-wiki-eval, llm-wiki-trust-audit |
...
| [`llm-wiki-ingestion-stack`](../skills/llm-wiki-ingestion-stack/SKILL.md) | design ingestion stack; convert documents to markdown; convert pdf to markdown; build ocr pipeline; plan incremental source sync | | none | yes | recommendation | wiki-ingest, llm-wiki-security-review |
...
| [`llm-wiki-local-first-stack`](../skills/llm-wiki-local-first-stack/SKILL.md) | local-first stack; offline wiki; local embeddings; avoid cloud lock-in | hybrid search; vector database choice; graphrag architecture; retrieval layer design | none | yes | recommendation | llm-wiki-setup, llm-wiki-obsidian-hardening, llm-wiki-retrieval-architect |
...
| [`llm-wiki-migration-planner`](../skills/llm-wiki-migration-planner/SKILL.md) | migration plan; dry-run refactor; move docs later | refactor docs; apply migration; organize vault now | none | no | plan-only | llm-wiki-refactor, llm-wiki-human-first-design, llm-wiki-obsidian-hardening |
...
| [`llm-wiki-news-radar`](../skills/llm-wiki-news-radar/SKILL.md) | find latest news; track fresh releases; summarize recent papers; monitor ecosystem changes | | none | yes | answer-with-citations | llm-wiki-ecosystem-registry, llm-wiki-choose |
...
| [`llm-wiki-provenance`](../skills/llm-wiki-provenance/SKILL.md) | missing sources; provenance gaps; unsupported claims; repair evidence links; investigate support labels | stable claim ids; source anchor ids; validate claim anchors; anti-slop controls; prompt-injection exposure | medium | no | report-or-patch | llm-wiki-claim-anchors, llm-wiki-conflict-resolver |
| [`llm-wiki-refactor`](../skills/llm-wiki-refactor/SKILL.md) | refactor docs; organize vault; apply migration; frontmatter normalization | no-write migration plan; dry-run plan only; plan before touching files | high | no | dry-run-first | wiki-lint, llm-wiki-provenance |
...
| [`llm-wiki-retrieval-architect`](../skills/llm-wiki-retrieval-architect/SKILL.md) | design retrieval layer; design hybrid search; choose vector database; design graphrag architecture; wiki does not fit in context | offline wiki; avoid cloud lock-in; local-first stack choice | none | yes | recommendation | llm-wiki-local-first-stack, llm-wiki-mcp-integration, llm-wiki-eval |
| [`llm-wiki-security-review`](../skills/llm-wiki-security-review/SKILL.md) | security review; is my setup safe; existing configuration safety; data boundary review; mcp exposure review; skill safety review | threat model; stride analysis; linddun analysis; data flow diagram; attack surface mapping; anti-slop controls; can the wiki be trusted | none | no | report-only | llm-wiki-model-policy, llm-wiki-privacy-redactor |
| [`llm-wiki-setup`](../skills/llm-wiki-setup/SKILL.md) | set up wiki; install workflow; bootstrap vault; configure agent skills; connect coding agent | zero to working wiki; first useful wiki; first demo loop | medium | no | plan-then-scaffold | llm-wiki-domain-pack, llm-wiki-obsidian-hardening, wiki-ingest |
...
| [`llm-wiki-trust-audit`](../skills/llm-wiki-trust-audit/SKILL.md) | trust audit; can the wiki be trusted; anti-slop controls; unsafe agent write permissions; prompt-injection exposure | is my setup safe; existing configuration safety; mcp exposure review; repair evidence links; missing sources; failure mode register; residual risk scorecard | none | no | report-only | llm-wiki-provenance, llm-wiki-critique-audit, wiki-lint |
| [`llm-wiki-zero-to-working-wiki`](../skills/llm-wiki-zero-to-working-wiki/SKILL.md) | zero to working wiki; first useful wiki; bootstrap first vault; first source ingest; first reusable answer; first lint report; demo wiki loop | install workflow; configure agent skills; connect coding agent | medium | no | plan-then-scaffold | llm-wiki-setup, wiki-ingest, wiki-query, wiki-lint, llm-wiki-eval |
